...
class Classifier {
  ...
  static Future<ClassifierModel> _loadModel(String modelFileName) async {
    try {
      debugPrint('Attempting to load model from asset: assets/models/$modelFileName');
  ...
  static Future<ClassifierLabels> _loadLabels(String labelsFileName) async {
    try {
      final labelsData = await rootBundle.loadString('assets/models/$labelsFileName');
  ...
  // Run inference on an input image and return all classification results
  List<ClassifierCategory> predictAll(img.Image image) {
    debugPrint(
    ...
    try {
      ...
      // Create an output buffer to store the model's predictions, assuming output shape [1, _labels.length]
      ...
      debugPrint('All categories: $resultCategories');
